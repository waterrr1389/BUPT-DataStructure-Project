...
/**
 * Creates a stable lookup key for an unordered node pair.
 */
function edgeKey(left, right) {
    return [left, right].sort().join("::");
}
/**
 * Resolves route-path presentation metadata with a walkway fallback.
 */
function getRoadTypeMeta(roadType) {
    return ROAD_TYPE_META[roadType] || ROAD_TYPE_META.walkway;
}
/**
 * Classifies a graph node as indoor or outdoor for styling and marker logic.
 */
...
/**
 * Scales destination graph coordinates into the fixed SVG canvas.
 */
...
/**
 * Computes a padded bounding box for a projected node set.
 */
...
/**
 * Groups destination buildings into projected overlay rectangles.
 */
...
/**
 * Builds the edge and node lookup tables used by route rendering.
 */
export function createRouteLookups(details) {
    return {
        edgeById: new Map(safeArray(details?.graph?.edges).map((edge) => [edge.id, edge])),
        edgeByPair: new Map(safeArray(details?.graph?.edges).map((edge) => [edgeKey(edge.from, edge.to), edge])),
        nodeById: new Map(safeArray(details?.graph?.nodes).map((node) => [node.id, node])),
    };
}
/**
 * Resolves a route step to the corresponding graph edge by id or endpoint pair.
 */
...
/**
 * Measures the turn angle between consecutive route nodes.
 */
...
/**
 * Derives route nodes, edge details, and marker-worthy transitions or turns.
 */
function analyzeRoute(details, route) {
    ...
    for (let index = 1; index < routeNodes.length - 1; index += 1) {
        ...
        const roadTypeChange = enteringStep?.edge?.roadType &&
...
/**
 * Positions the text pill that accompanies an on-map marker.
 */
...
/**
 * Positions the compact legend pill used beside marker icons.
 */
...
/**
 * Renders the shape primitive for an active or preview route marker.
 */
...
/**
 * Wraps marker geometry and pill copy in the shared group markup.
 */
...
/**
 * Flattens the separated endpoint, transition, and turn markers into draw order.
 */
...
/**
 * Renders all active-route markers into a single SVG fragment.
 */
...
/**
 * Renders preview-only markers when no active route is highlighted.
 */
...
/**
 * Renders the legend icon for a route path segment type.
 */
...
/**
 * Renders the legend icon for a route marker sample.
 */
...
/**
 * Wraps a legend icon and label in the shared badge markup.
 */
...
/**
 * Produces the legend rows for either an active route or a preview-only state.
 */
export function buildRouteLegendItems(routeAnalysis, markerLayout, previewMarkers) {
    if (routeAnalysis) {
        const renderedRoadTypes = new Set(routeAnalysis.stepDetails.map((step) => step.edge?.roadType || "walkway"));
        ...
        const startMarker = safeMarkerLayout.endpointMarkers.find((marker) => marker.kind === "start") || null;
        ...
        const activeLegendMarkers = ACTIVE_MARKER_LEGEND_ORDER.flatMap((kind) => (activeMarkerSamples[kind] ?? []).filter(Boolean));
        return [
...
/**
 * Precomputes the projection, overlays, and node options for one destination.
 */
function createScene(details) {
    ...
    return {
        projection,
        lookups,
        outdoorBounds: createBounds(safeArray(details?.graph?.nodes).filter((node) => getNodeEnvironment(node) === "outdoor"), projection, 36),
...
/**
 * Reads or creates the cached scene projection for a destination map.
 */
...
/**
 * Renders the SVG route visualization while preserving preview and active marker semantics.
 */
export function renderRouteVisualization(options) {
    ...
    const routeCueMarkup = routeAnalysis && (routeAnalysis.transitionMarkers.length > 0 || routeAnalysis.turnMarkers.length > 0)
...
/**
 * Renders the textual route result card for a planned destination route.
 */
