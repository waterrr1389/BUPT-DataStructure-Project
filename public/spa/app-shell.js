// @ts-nocheck
import { createRouteContextHref, debounce, emptyStateMarkup, escapeHtml, fillSelect, isPrimaryNavigationEvent, noticeMarkup, resultMetaMarkup, safeArray, tagsMarkup, text, } from "./lib.js";
/**
...
export function createAppShell(root) {
    ...
    async function loadBootstrap() {
        ...
        if (!state.bootstrapPromise) {
            state.bootstrapPromise = (async () => {
                ...
                const prepared = journalConsumers.prepareDestinationSelectorBindings(bootstrap, journalPresentation.createDestinationSelectOptions);
                const journalPrepared = journalConsumers.prepareJournalExchangeDestinationBindings(bootstrap, journalPresentation.createDestinationSelectOptions);
                ...
                setStatus(`Runtime data: ${text(bootstrap?.source?.data, "seeded")}. Algorithms: ${text(bootstrap?.source?.algorithms, "fallback")}.`, "success");
                return bootstrap;
            })().catch((error) => {
    ...
    async function fetchFeed(filters = {}) {
        ...
        const fallback = await requestJson(`/api/journals${fallbackParams.toString() ? `?${fallbackParams.toString()}` : ""}`);
        return {
    ...
    async function fetchJournalComments(journalId, options = {}) {
        ...
        const response = await requestJsonMaybe(`/api/journals/${encodeURIComponent(journalId)}/comments${params.toString() ? `?${params.toString()}` : ""}`);
        if (response.ok) {
            state.socialAvailability.comments = true;
            return {
                available: true,
                items: safeArray(response.payload?.items),
                nextCursor: typeof response.payload?.nextCursor === "string" ? response.payload.nextCursor : "",
    ...
    async function renderRoute(options = {}) {
        ...
        setLoadingState(route.name === "notFound" ? "Resolving route" : `Opening ${titleByRoute[route.name] || "view"}`, "Loading only the current surface to keep the shell responsive.");
        try {
