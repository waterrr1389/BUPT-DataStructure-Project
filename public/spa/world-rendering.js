// @ts-nocheck
import { createRouteContextHref, emptyStateMarkup, escapeHtml, resultMetaMarkup, resolveRouteActor, safeArray, text, } from "./lib.js";
...
function worldRoutePoints(world, itinerary) {
    const nodeById = new Map(safeArray(world?.graph?.nodes).map((node) => [text(node?.id), node]));
...
function worldRouteExplanationMarkup(itinerary) {
    ...
    <ol class="world-route-explanation-list" data-route-world-explanation="true">
      ${segments
        .map((segment) => `<li data-route-world-explanation-segment="${escapeHtml(segment.kind)}" data-route-world-explanation-order="${segment.order}">${escapeHtml(segment.summary)}</li>`)
        .join("")}
    </ol>
  `;
}
...
function createWorldRouteLocalHref(leg, itinerary, route) {
    ...
    const localNodeIds = safeArray(leg?.localNodeIds).map((nodeId) => text(nodeId)).filter(Boolean);
    return createRouteContextHref("/map", {
        destinationId,
        from: localNodeIds[0] || "",
...
function worldRouteResultMarkup(itinerary, route) {
    ...
    const toLocalLeg = text(itinerary?.scope) === "cross-map" && destinationLegs.length > 1
...
function worldMetaMarkup(summary, world) {
    ...
    const destinationCount = safeArray(world?.destinations).length || safeArray(summary?.destinations).length;
    return `
  ...
  `;
    ...
    try {
        ...
        if (summary?.enabled !== true) {
            ...
            renderUnavailable("World map unavailable", "World mode is not enabled by the backend worker in this workspace.");
        ...
        if (issues.length > 0) {
            renderUnavailable("World details unavailable", "World details payload failed validation. Check world bounds, polygons, markers, and route graph data.");
            app.setStatus(`World details are malformed: ${issues[0]}`, "error");
            return () => { };
        }
        if (!world) {
            renderUnavailable("World details unavailable", "The world surface is enabled, but detailed map data is missing.");
        ...
        if (toDestinationSelect) {
            toDestinationSelect.innerHTML = selectOptionsMarkup(destinationOptions, destinationOptions[1]?.id || destinationOptions[0]?.id);
        ...
        renderUnavailable("World details unavailable", "The world surface could not be prepared. Check the worker endpoints and try again.");
        app.setStatus(message, "error");
    }
    return () => {
        disposed = true;
