// @ts-nocheck
import { createRouteContextHref, emptyStateMarkup, fillSelect, parseListInput, resolveRouteActor, safeArray, } from "../lib.js";
/**
...
export async function render(app, route, root) {
  ...
  `;
    ...
    function updateRouteQuery(replace = true) {
        ...
        app.navigate(buildContextualMapHref(params), { replace, preserveScroll: true, render: false });
    }
    ...
    async function renderMapSurface(activeRoute = currentRoute) {
        ...
        if (!destinationId) {
            renderInlineMapState("Choose a destination", "The map surface loads only after a destination is selected.");
            return;
        }
        let details;
        try {
            ...
            app.setStatus(error instanceof Error ? error.message : "Map preview failed.", "error");
            renderInlineMapState("Map unavailable", "Destination data could not be loaded. Choose another destination to keep routing.");
            return;
        }
        if (disposed || !details) {
            return;
        ...
        routeResult.innerHTML =
            activeRoute && activeRoute.destinationId === destinationId
                ...
                : mapStageEmptyMarkup("Route summary appears after planning", "Preview markers update while you adjust the route, and distance plus directions appear after you plan it.");
    }
    ...
    const debouncedPreview = app.debounce(() => {
        currentRoute = null;
        void renderMapSurface(null).catch((error) => app.setStatus(error instanceof Error ? error.message : "Map preview failed.", "error"));
        updateRouteQuery(true);
    }, 180);
