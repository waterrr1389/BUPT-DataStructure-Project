...
(function attachJournalPresentation(root, factory) {
     ...
     */
    function createSelectOptionsWithDisambiguatedLabels(items, { getKey = (item) => item?.id, getBaseLabel = (item) => item?.name ?? item?.id, getQualifierParts = (item) => [item?.id], } = {}) {
    ...
    function formatJournalMetadata(journal, lookups = {}) {
        const destinationLabel = resolveLookupLabel(journal?.destinationId, lookups.destinationById, "Unknown destination");
