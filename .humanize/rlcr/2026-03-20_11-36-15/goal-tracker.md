...
| Centralize legend and route marker semantics (renderRouteVisualization + marker helpers) so legend items and SVG rendering draw from the same mapping and existing tests stay green | AC-2 | pending | Will reconcile Start/End/route swatches and keep `route-visualization-markers.test.ts` reliable |
| Restructure the left control panel: promote `Route Planning` to the dominant title, demote auxiliary copy, and group `Destination`, `Start node`, and `End node` into a cohesive spatial context block | AC-3 | pending | Aimed at DOM/visual grouping changes in `public/spa/views/map.js` |
...
| Remove the `Calm Empty State` label, strip developer-focused deep-link copy from hero/body, and ensure the empty state helper only surfaces user-facing text | AC-5 | pending | Will touch empty state helper and verify no internal names reach the UI |
