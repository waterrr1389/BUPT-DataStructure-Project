...
### Plan Version: 3 (Updated: Round 1 Review)
...
| AC-2 | In progress | Core design, requirements, task, user, and reference docs mostly align on repository structure, module boundaries, command surface, and current capability statements, including `public/**` versus `dist/public/**`, the broader `scripts/**` surface, and the implemented browser/API endpoints, but review reopened two precision fixes in `docs/requirements-analysis.md` and `docs/module-design.md`. |
...
| Milestone 3: align the main delivery docs | In progress | The core design/requirements/task/user/reference docs were updated across Rounds 0-1, but review reopened the remaining evidence-attribution fix in `docs/requirements-analysis.md` and the remaining browser asset-tree inventory fix in `docs/module-design.md`. |
...
|------|-----------|--------|-------|
| Tighten the March 27 selector-parity and graph-variant evidence attribution in `docs/requirements-analysis.md` | AC-2 | Open | Keep the March 19 versus March 27 boundary fix, but name the actual test coverage precisely instead of attributing it to broader suite buckets. |
| Add the first-party browser asset tree to the module inventory in `docs/module-design.md` | AC-2 | Open | The module layout list still needs `public/assets/**` and `dist/public/assets/**` so the structure inventory matches the repository and the later asset-copy description. |
...
| AC-2, AC-3 | Aligned most of the core design, requirements, task, user, and reference documents to the current repository | 0-1 | 1 | `README.md`, `docs/overall-design.md`, `docs/task-description.md`, `docs/user-guide.md`, `docs/example-results-and-tests.md`, `docs/evaluation-and-improvements.md`, `docs/innovation-notes.md`, `docs/data-structures-and-dictionary.md`, and `docs/journal-social-design-style.md` align on structure, paths, commands, surfaces, and dated evidence boundaries; Round 1 review left two AC-2 precision follow-ups open in `docs/module-design.md` and `docs/requirements-analysis.md`. |
...
|-------|-----------------|-------------|-----------------|
| `docs/requirements-analysis.md` still attributes the March 27 selector-parity and graph-variant evidence to broader suite buckets than the current tests actually provide. | 1 review | AC-2 | Rewrite the March 27 evidence bullets to cite the selector-binding/browser-side coverage and the runtime-service graph-variant coverage precisely while preserving the corrected March 19 versus March 27 boundary. |
| `docs/module-design.md` still omits `public/assets/**` and `dist/public/assets/**` from the module inventory even though the document later describes those copied assets. | 1 review | AC-2 | Expand the module-layout list so the first-party asset tree is represented explicitly and the document no longer understates the current browser asset structure. |
