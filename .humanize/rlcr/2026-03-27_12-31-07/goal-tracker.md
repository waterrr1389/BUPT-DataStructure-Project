...
### Plan Version: 5 (Updated: Round 5 review)
...
| 5 | Accepted the FR-2 / FR-3 evidence split and a fresh `npm test` rerun, but kept the docs/bookkeeping batch open because the repo-wide verification narrative is still not fully aligned. | Round 5 fixed the earlier FR-2 / FR-3 mismatch, but `docs/requirements-analysis.md` still mixes March 27 rerun text into the FR-6 March 19 block, the round summaries retain stale repo-state snapshots, and the Round 5 README grep claim does not match current HEAD. | 5, 7 |

#### Active Tasks
<!-- Map each task to its target Acceptance Criterion -->
| Task | Target AC | Status | Notes |
|------|-----------|--------|-------|
| Reconcile `docs/requirements-analysis.md`, `round-4-summary.md`, `round-5-summary.md`, and the mutable bookkeeping with the verified current state. | 5, 7 | Narrowed | The guard diagnostics, child-process cwd proof, and `npm test` rerun are now verified on current HEAD, and the FR-2 / FR-3 attribution mismatch is fixed. Remaining work is documentation/bookkeeping only: FR-6 still places a March 27 rerun bullet inside a March 19 evidence block, both round summaries still report the earlier ahead-count / modified-tracker workspace state, and Round 5's `rg -n \"March 27\" ...` claim overstates README coverage because README uses a Chinese-formatted date. Keep AC-7 explicit in mutable tracking because the immutable section cannot be edited. |
...
| Docs update | Review README and delivery docs for `dist/public`, static-copy behavior, and `public/vendor/**` exception wording | README and the main delivery docs distinguish the March 27 current-HEAD rerun from the March 19 and March 18 historical records, and `docs/requirements-analysis.md` now splits FR-2 / FR-3 correctly, but FR-6 plus the Round 4 / Round 5 summaries still contain stale verification/bookkeeping wording. | Reopened | Keep this batch open until the FR-6 evidence labels and the summary bookkeeping statements match the verified current-HEAD wording. |
...
|-------|-----------------|-------------|-----------------|
| `docs/requirements-analysis.md` still places the March 27 rerun note for selector-parity and graph-variant coverage under the FR-6 March 19 evidence block, so the repo-wide verification narrative remains mixed. | 5 | 5 | Split the FR-6 March 19 and March 27 evidence into separate labeled blocks so the file-level verification framing stays internally consistent. |
| `round-4-summary.md` and `round-5-summary.md` still report the earlier ahead-count / modified-tracker workspace state, and `round-5-summary.md` overclaims that `rg -n \"March 27\" ...` confirms README coverage even though README uses `2026 年 3 月 27 日`. | 5 | 5, 7 | Refresh the summary repo-state wording and adjust the README consistency-check claim to match the actual command output before closing docs/bookkeeping. |
