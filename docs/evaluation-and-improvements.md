...
- The project stays inspectable while relying on `leaflet` for browser maps, `busboy` for multipart image uploads, and TypeScript tooling (`@types/busboy`, `@types/leaflet`, `typescript`) during builds without overstating its runtime dependency profile.
